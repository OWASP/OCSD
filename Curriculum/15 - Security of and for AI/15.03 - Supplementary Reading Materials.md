...
The [Input Threats section of the OWASP AI Exchange](https://owaspai.org/docs/2_threats_through_use/#rate-limit) highlights how limiting the rate of access to the model - preferably per actor (user, API key or session) may not only to prevent resource exhaustion but also to severely slow down experimentation that underlies many AI attacks through use. You can also find relevant information from [OpenCRE](https://opencre.org/cre/187-083) and [MITRE Atlas](https://atlas.mitre.org/mitigations/AML.M0004).

### Supply Chain Risks for AI Models

Further information about supply chain security for AI models can be found [Chapter 6](https://github.com/OWASP/AISVS/blob/main/1.0/en/0x10-C06-Supply-Chain.md) of the OWASP Artificial Intelligence Security Verification Standard.
